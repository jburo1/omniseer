...
HwMotorDriver::HwMotorDriver(TwoWire& wire, uint8_t i2c_addr)
    : _wire{wire}
    , _i2c_addr{i2c_addr}
{}
...
void HwMotorDriver::set_wheel_speeds_rad_s(const WheelSpeeds& ws) {
    ...
    for (uint8_t wheel_idx = 0; wheel_idx < 4; ++wheel_idx) {
        ...
        if (driver_units >  DRIVER_SPEED_LIMIT) driver_units =  DRIVER_SPEED_LIMIT;
        if (driver_units < -DRIVER_SPEED_LIMIT) driver_units = -DRIVER_SPEED_LIMIT;
