...
| Bounded visual target acquisition and framing | **Implemented; one public physical-run artifact** | `omniseer_autonomy`, `scripts/omni run autonomy`, controller tests, and RunBundle `autonomy.jsonl` support; the [v2-L FP target-acquisition study](https://github.com/jburo1/omniseer/tree/master/studies/autonomy/v2l_fp_target_acquisition) retains one complete successful physical-run RunBundle. The detector comparison is separate perception evidence, not an autonomy RunBundle. |
