...
| [v2-L FP target acquisition](target-acquisition.md) | One ROCK 5B+ execution completed bounded visual target acquisition and framing | Complete `v2l_fp_scene_1` RunBundle: manifest, autonomy trace, detections, telemetry, evidence frames, video, rosbag, provenance, and static report | ROCK 5B+ physical robot | Yes | One successful representative run; not a benchmark, statistical result, navigation-based search, global exploration, learned control, or general detector-accuracy claim |
| [v2-L INT8 quantization failure analysis](int8-quantization-failure.md) | A fixed RK3588 evaluation observed recalibrated INT8 collapse and partial mixed-precision recovery | 300-frame summary, retained replay, provenance, projection-context metrics, and evidence inventory | RK3588 target hardware | Yes, except non-retained source inputs | Fixed replay only; not broad deployment validation, ground-truth accuracy, isolated latency measurement, or proof of a Toolkit2/backend root cause |
...
The [v2-L FP target-acquisition RunBundle](target-acquisition.md)
