...
Can open-vocabulary YOLO-World v2-L FP running through RKNN on a ROCK 5B+ support a complete, reviewable execution of Omniseer's bounded visual target-acquisition and framing behavior?

## Observed physical run

The public `v2l_fp_scene_1` RunBundle records one successful physical-robot execution for the configured `person` target. The controller scanned, acquired a stable detection, centered and framed it, then stopped with terminal reason `framed`.
