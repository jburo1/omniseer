...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::msg::LaserScan::SharedPtr new_scan)
{
  if (GT_pose_initialized)
  {
    ...
    // if (rf2o_ref.first_laser_scan == true)
    // {
    ...
    if (rf2o_ref.first_laser_scan) {
      ...
      rf2o_ref.init(last_scan, initial_robot_pose.pose.pose);
      rf2o_ref.first_laser_scan = false;
      return;                 // do NOT signal a new scan on the init frame
    }
    new_scan_available = true;  // only from the second scan onward
  }
}
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rf2o::CLaserOdometry2DNode>() ;
  rclcpp::Rate rate(node->freq);
  while (rclcpp::ok()){
      node->process();
      rclcpp::spin_some(node);
      try {
        rate.sleep();  // returns promptly after Ctrl-C
      } catch (const rclcpp::exceptions::RCLError &e) {
        RCLCPP_WARN(node->get_logger(), "Rate sleep interrupted: %s", e.what());
        break;
      }
  }
  rclcpp::shutdown();
