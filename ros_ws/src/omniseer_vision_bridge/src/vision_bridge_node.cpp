...
namespace omniseer_vision_bridge
{
  ...
  {
  public:
    RosYoloDetectionsSink(rclcpp::Publisher<yolo_msgs::msg::DetectionArray>::SharedPtr publisher,
  ...
  {
  public:
    VisionBridgeNode() : rclcpp::Node("vision_bridge")
    {
      ...
      _detections_sink = std::make_unique<RosYoloDetectionsSink>(
          _detections_publisher, omniseer::vision::load_class_list_file(_config.class_list_path),
          _config.camera_frame_id);
      _perf_publisher = create_publisher<omniseer_msgs::msg::VisionPerfSummary>("/vision/perf", 1);
      ...
      _capture_service = create_service<omniseer_msgs::srv::CaptureFrame>(
          ...
          [this](const std::shared_ptr<omniseer_msgs::srv::CaptureFrame::Request> request,
                 std::shared_ptr<omniseer_msgs::srv::CaptureFrame::Response>      response)
          { handle_capture_request(*request, *response); });
      RCLCPP_INFO(get_logger(), "vision runtime started in headless bridge mode and publishing "
      ...
      _perf_timer =
          create_wall_timer(std::chrono::milliseconds(500), [this]() { publish_perf_summary(); });

      _health_timer = create_wall_timer(std::chrono::milliseconds(500),
                                        [this]()
                                        {
  ...
  private:
    void handle_capture_request(const omniseer_msgs::srv::CaptureFrame::Request& request,
                                omniseer_msgs::srv::CaptureFrame::Response&      response)
    {
      ...
      metadata.capture_reason =
          request.capture_reason.empty() ? "target_framed" : request.capture_reason;
    ...
    void publish_perf_summary()
    {
      ...
      {
        producer_fps =
            static_cast<double>(snapshot.produced_count - _last_perf_snapshot.produced_count) / dt;
        consumer_fps =
            static_cast<double>(snapshot.consumed_count - _last_perf_snapshot.consumed_count) / dt;
      }
      ...
      msg.last_source_age_end_ms   = static_cast<float>(ns_to_ms(snapshot.last_source_age_end_ns));
    ...
    VisionBridgeRuntimeConfig declare_bridge_parameters()
    {
      ...
      cfg.class_list_path = declare_parameter<std::string>("classes.path", cfg.class_list_path);
      cfg.pad_token       = declare_parameter<std::string>("classes.pad_token", cfg.pad_token);

      cfg.producer_preflight_capture_wait_ms =
          declare_parameter<int64_t>("producer.preflight_capture_wait_ms",
                                     cfg.producer_preflight_capture_wait_ms);
    ...
    void log_config(const VisionBridgeRuntimeConfig& cfg) const
    {
      ...
      message << "configured bridge:"
              << " camera.device=" << cfg.camera_device << " camera.width=" << cfg.camera_width
