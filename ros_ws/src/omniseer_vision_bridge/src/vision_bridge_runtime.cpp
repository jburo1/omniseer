...
namespace omniseer_vision_bridge
{
  namespace
  {
    template <typename T> uint32_t checked_u32(T value, const char* name)
    {
      if (value <= 0)
      {
    ...
    template <typename T> uint32_t checked_non_negative_u32(T value, const char* name)
    {
      if (value < 0)
      {
        throw std::invalid_argument(std::string(name) + " must be >= 0");
  ...
  {
    explicit Impl(VisionBridgeRuntimeConfig config_in) : config(std::move(config_in)) {}
    ...
    void start()
    {
      ...
      try
      {
        ...
        capture_config.buffer_count =
            checked_u32(config.camera_buffer_count, "camera.buffer_count");
        ...
        pool->allocate_all(
            *allocator,
            ...
            static_cast<int>(checked_u32(config.pipeline_dst_height, "pipeline.dst_height")),
            omniseer::vision::PixelFormat::RGB888);

        omniseer::vision::RgaPreprocessConfig preprocess_config{};
        preprocess_config.src_w =
            static_cast<int>(checked_u32(config.camera_width, "camera.width"));
        preprocess_config.src_h =
            static_cast<int>(checked_u32(config.camera_height, "camera.height"));
        preprocess_config.dst_w =
            static_cast<int>(checked_u32(config.pipeline_dst_width, "pipeline.dst_width"));
        preprocess_config.dst_h =
            static_cast<int>(checked_u32(config.pipeline_dst_height, "pipeline.dst_height"));
        ...
        producer_config.preflight_capture_wait_ms =
        ...
        omniseer::vision::ITelemetry* telemetry_sink =
            telemetry ? static_cast<omniseer::vision::ITelemetry*>(telemetry.get())
                      : &rolling_stats;
        producer =
            std::make_unique<omniseer::vision::ProducerPipeline>(*capture, *preprocess, *pool,
                                                                 telemetry_sink, producer_config);
        producer->preflight();
        ...
        text_builder =
            std::make_unique<omniseer::vision::YoloWorldTextEmbeddingsBuilder>(embeddings_config);
        ...
        consumer_config.score_threshold =
            checked_unit_float(config.score_threshold, "postprocess.score_threshold");
        consumer_config.nms_iou_threshold =
            checked_unit_float(config.nms_iou_threshold, "postprocess.nms_iou_threshold");
        consumer_config.max_detections =
            checked_u32(config.max_detections, "postprocess.max_detections");
        consumer =
            std::make_unique<omniseer::vision::ConsumerPipeline>(*pool, *runner, telemetry_sink,
                                                                 config.detections_sink,
                                                                 consumer_config);
  ...
  omniseer::vision::RollingTelemetrySnapshot VisionBridgeRuntime::telemetry_snapshot()
      const noexcept
  {
