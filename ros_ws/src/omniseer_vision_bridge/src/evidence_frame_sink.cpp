...
namespace omniseer_vision_bridge
{
  namespace
  {
    ...
    bool write_jpeg(const std::filesystem::path& path, const std::vector<uint8_t>& rgb, int width,
                    int height, int quality) noexcept
    {
  ...
  struct EvidenceFrameSink::Impl
  {
    ...
    struct CaptureWaiter
    {
      explicit CaptureWaiter(TargetCaptureMetadata metadata_in) : metadata(std::move(metadata_in))
      {
      }
    ...
    explicit Impl(EvidenceFrameSinkConfig config_in)
        : config(std::move(config_in)), sample_interval_ns(interval_ns(config.interval_sec)),
    ...
    void stop_with_reason(const std::string& reason) noexcept
    {
      ...
      metadata << "{\"schema_version\":" << kEvidenceSchemaVersion
               ...
               << json_escape(reason)
               << "\",\"dropped_count\":" << dropped_count.load(std::memory_order_relaxed) << "}\n";
      metadata.flush();
    }
    ...
    void complete_capture(const std::shared_ptr<CaptureWaiter>& waiter, bool success,
                          std::string reason, const Item& item,
    ...
    void write_metadata(const Item& item, const std::string& image_path)
    {
      metadata << "{\"schema_version\":" << kEvidenceSchemaVersion
               ...
               << ",\"jpeg_quality\":" << jpeg_quality << ",\"capture_reason\":\""
               << json_escape(item.capture_reason) << "\""
               ...
               << ",\"remap\":{\"scale\":" << item.remap_scale << ",\"pad_x\":" << item.remap_pad_x
               << ",\"pad_y\":" << item.remap_pad_y << ",\"resized_w\":" << item.remap_resized_w
      ...
      for (std::size_t i = 0; i < item.detections.size(); ++i)
      {
        ...
        metadata << "{\"class_id\":" << det.class_id << ",\"class_name\":\""
  ...
  void EvidenceFrameSink::publish(const omniseer::vision::ImageBuffer&         image,
                                  const omniseer::vision::DetectionsFrame&     detections,
                                  const omniseer::vision::PipelineRemapConfig& remap) noexcept
  {
