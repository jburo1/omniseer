...
def generate_launch_description():
    declared_arguments = [
        ...
        DeclareLaunchArgument("log_level", default_value="info", description="debug|info|warn|error|fatal"),
    ]
    ...
    sensor_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([PathJoinSubstitution([pkg_bringup, "launch", "sensors.launch.py"])]),
        launch_arguments={"use_sim_time": use_sim_time, "log_level": log_level}.items(),
    )
    ...
    on_clock = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[LogInfo(msg="/clock found"), spawn_robot_launch, wait_tf_static],
        )
    )
    ...
    on_odom_map_tf = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[LogInfo(msg="map->base_link tf found"), nav_launch, rviz_launch],
        )
    )
