...
namespace omniseer_autonomy
{
  ...
  public:
    ...
    TargetCenteringOutput update_detections(const std::vector<TargetDetection>& detections,
                                            double                              now_sec);
  ...
  private:
    ...
    void transition(CenteringState state, double now_sec, TargetCenteringOutput& output,
    ...
    TargetCenteringEvent make_event(double now_sec, std::string event, std::string reason = "",
                                    std::optional<TargetDetection> target           = std::nullopt,
                                    std::optional<double>          normalized_error = std::nullopt,
                                    std::optional<double>          bbox_area_ratio  = std::nullopt,
                                    double linear_x_m_s = 0.0, double angular_z_rad_s = 0.0) const;
