...
void TargetCenteringController::transition(
  CenteringState state, double now_sec, TargetCenteringOutput & output,
  std::string event, std::string reason, std::optional<TargetDetection> target)
{
  _state = state;
  _state_started_at_sec = now_sec;
  output.events.push_back(make_event(
    now_sec, std::move(event), std::move(reason),
...
TargetCenteringOutput TargetCenteringController::command_for_target(
  const TargetDetection & target, double now_sec)
{
  TargetCenteringOutput output{};
