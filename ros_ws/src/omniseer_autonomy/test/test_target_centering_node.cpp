...
namespace omniseer_autonomy
{
  ...
  TEST_F(TargetCenteringNodeTest, WritesEventsAndRequestsCaptureOnSuccess)
  {
    ...
    auto capture_server = helper->create_service<omniseer_msgs::srv::CaptureFrame>(
        capture_service,
        [&capture_seen](const std::shared_ptr<omniseer_msgs::srv::CaptureFrame::Request> request,
    ...
    auto detections =
        helper->create_publisher<yolo_msgs::msg::DetectionArray>(detections_topic, 10);
    ...
    ASSERT_TRUE(spin_until(executor,
                           [&]()
                           {
                             return capture_seen && std::filesystem::exists(log_path) &&
                                    read_text(log_path).find("\"event\":\"capture_result\"") !=
                                        std::string::npos;
                           }));
