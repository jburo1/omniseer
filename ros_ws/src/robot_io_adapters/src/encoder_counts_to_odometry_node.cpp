...
namespace robot_io_adapters
{
      ...
      : rclcpp::Node("encoder_counts_to_odometry")
  {
    ...
    _odom_topic =
        this->declare_parameter<std::string>("odom_topic", "/mecanum_drive_controller/odometry");
    ...
    const double encoder_ticks_per_rev =
        this->declare_parameter<double>("encoder_ticks_per_rev", 44.0 * 131.0);
    ...
    _integrator = std::make_unique<EncoderOdometryIntegrator>(wheel_radius_m, half_length_m,
                                                              half_width_m, encoder_ticks_per_rev);
    ...
    _subscription = this->create_subscription<omniseer_msgs::msg::WheelEncoderCounts>(
        _encoder_topic, 10, [this](const omniseer_msgs::msg::WheelEncoderCounts& message)
        { encoder_callback(message); });

    RCLCPP_INFO(this->get_logger(), "EncoderCountsToOdometry: %s -> %s", _encoder_topic.c_str(),
                _odom_topic.c_str());
  }
      ...
      const omniseer_msgs::msg::WheelEncoderCounts& message)
  {
    ...
    const WheelCountsSnapshot sample{
        stamp_s, message.front_left, message.front_right, message.rear_left, message.rear_right,
    };
  ...
  std::array<double, 36> EncoderCountsToOdometryNode::diagonal_covariance(double x, double y,
