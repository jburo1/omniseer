...
namespace robot_io_adapters
{
  ...
  double counts_delta_to_wheel_angular_velocity(int64_t delta_counts, double dt_s,
  ...
  BodyTwist body_twist_from_wheel_angular_velocities(double front_left, double front_right,
                                                     double rear_left, double rear_right,
                                                     double wheel_radius_m, double half_length_m,
                                                     double half_width_m)
  {
  ...
  PlanarPose integrate_planar_pose(const PlanarPose& pose, const BodyTwist& twist, double dt_s)
  {
    if (dt_s <= 0.0)
    {
      throw std::invalid_argument("dt_s must be positive");
  ...
  EncoderOdometryIntegrator::EncoderOdometryIntegrator(double wheel_radius_m, double half_length_m,
                                                       double half_width_m,
                                                       double encoder_ticks_per_rev)
      : _wheel_radius_m(wheel_radius_m), _half_length_m(half_length_m), _half_width_m(half_width_m),
      ...
      const WheelCountsSnapshot& sample)
  {
    ...
    const double front_left_rate =
        counts_delta_to_wheel_angular_velocity(sample.front_left - _previous_sample->front_left,
                                               dt_s, _encoder_ticks_per_rev);
    const double front_right_rate =
        counts_delta_to_wheel_angular_velocity(sample.front_right - _previous_sample->front_right,
                                               dt_s, _encoder_ticks_per_rev);
    const double rear_left_rate =
        counts_delta_to_wheel_angular_velocity(sample.rear_left - _previous_sample->rear_left, dt_s,
                                               _encoder_ticks_per_rev);
    const double rear_right_rate =
        counts_delta_to_wheel_angular_velocity(sample.rear_right - _previous_sample->rear_right,
                                               dt_s, _encoder_ticks_per_rev);

    const BodyTwist twist =
        body_twist_from_wheel_angular_velocities(front_left_rate, front_right_rate, rear_left_rate,
                                                 rear_right_rate, _wheel_radius_m, _half_length_m,
                                                 _half_width_m);
