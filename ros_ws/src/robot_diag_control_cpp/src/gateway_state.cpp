...
namespace robot_diag_control_cpp
{
  ...
  GatewayStateStore::GatewayStateStore(std::string gateway_name, std::string gateway_version,
                                       std::chrono::milliseconds vision_stale_after,
                                       std::chrono::milliseconds odom_stale_after,
  ...
  PreviewStatusSnapshot GatewayStateStore::set_preview_disabled(PreviewProfile profile,
  ...
  void GatewayStateStore::update_vision_perf(const omniseer_msgs::msg::VisionPerfSummary& msg)
  {
    ...
    _vision_perf     = StoredVisionPerf{
        static_cast<double>(msg.producer_fps),  static_cast<double>(msg.consumer_fps),
        static_cast<double>(msg.last_infer_ms), msg.infer_error_count,
        msg.capture_fatal_error_count,          _time_source(),
    };
  }
  ...
  void GatewayStateStore::update_detections(const yolo_msgs::msg::DetectionArray& msg)
  {
    ...
    {
      _detections.detections.push_back(DetectionOverlayItem{
          detection.class_id,
  ...
  DetectionOverlaySnapshot GatewayStateStore::detection_overlay_snapshot_locked() const
  {
    if (!_has_detections)
    {
      return DetectionOverlaySnapshot{
          false, false, 0, _detection_source_width_px, _detection_source_height_px, {},
      };
    }
  ...
  std::vector<OperatorEventSnapshot> GatewayStateStore::operator_events_snapshot_locked() const
  {
    ...
    {
      events.push_back(OperatorEventSnapshot{
          event.sequence,
          static_cast<uint64_t>(
              std::chrono::duration_cast<std::chrono::milliseconds>(now - event.created_at)
                  .count()),
          event.message,
      });
    }
      ...
      const PlatformStatusSnapshot& platform) const
  {
    ...
    {
      append_operator_event_locked(preview_error_active ? "preview error: " + preview.last_error
                                                        : "preview recovered");
    }
    _last_preview_error_active = preview_error_active;
    ...
    {
      append_operator_event_locked(teleop_error_active ? "teleop error: " + teleop.last_error
                                                       : "teleop command recovered");
    }
    ...
    {
      append_operator_event_locked(compute_fault_active ? "compute degraded" : "compute recovered");
    }
    _last_compute_fault_active = compute_fault_active;

    const auto& lipo = platform.power.lipo_battery;
    const bool  lipo_low_active =
        lipo.available && !lipo.stale && lipo.voltage_available && lipo.voltage <= kLowLipoVoltage;
  ...
  void GatewayStateStore::append_operator_event_locked(std::string message) const
  {
    ...
    _operator_events.push_back(StoredOperatorEvent{
