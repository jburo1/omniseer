...
namespace robot_diag_control_cpp
{
  namespace
  {
    PreviewStatusSnapshot preview_disabled(GatewayStateStore& store, PreviewProfile profile,
  ...
  PreviewControlResult PreviewProcessManager::set_enabled(bool                          enabled,
                                                          std::optional<PreviewProfile> profile)
  {
    std::lock_guard<std::mutex> lock(_mutex);
    poll_locked();
  ...
  PreviewControlResult PreviewProcessManager::enable_locked(PreviewProfile target_profile)
  {
    ...
    {
      const auto preview =
          preview_disabled(_store, current.profile, "preview process did not stop cleanly");
    ...
    {
      const auto preview =
          preview_disabled(_store, target_profile, "preview command not configured");
      return PreviewControlResult{false, preview.last_error, preview};
    }
    ...
    {
      const auto message =
          std::string("failed to create preview exec pipe: ") + std::strerror(errno);
      const auto preview = preview_disabled(_store, target_profile, message);
      return PreviewControlResult{false, message, preview};
    }
    ...
    if (child_pid < 0)
    {
      ...
      close(exec_error_pipe[1]);
      const auto message =
          std::string("failed to fork preview process: ") + std::strerror(saved_errno);
      const auto preview = preview_disabled(_store, target_profile, message);
      return PreviewControlResult{false, message, preview};
    }
    ...
    if (child_pid == 0)
    {
      ...
      if (dup2(devnull_fd, STDIN_FILENO) < 0 || dup2(devnull_fd, STDOUT_FILENO) < 0 ||
  ...
  bool PreviewProcessManager::child_exited_locked(int& wait_status)
  {
    ...
    if (result < 0)
    {
      ...
      const auto current = _store.get_preview_status();
      _child_pid         = -1;
      _store.set_preview_disabled(current.profile, std::string("failed to poll preview process: ") +
                                                       std::strerror(errno));
      return false;
    }
