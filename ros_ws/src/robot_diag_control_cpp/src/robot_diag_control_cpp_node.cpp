...
namespace robot_diag_control_cpp
{
  ...
  {
  public:
    RobotDiagControlCppNode() : rclcpp::Node("robot_diag_control_cpp")
    {
      const auto vision_stale_after_ms = declare_parameter<int64_t>("vision_stale_after_ms", 2000);
      const auto odom_stale_after_ms   = declare_parameter<int64_t>("odom_stale_after_ms", 1000);
      ...
      const auto grpc_bind_address = declare_parameter<std::string>("grpc_bind_address", "0.0.0.0");
      ...
      const auto preview_source_kind =
          declare_parameter<std::string>("preview_source_kind", "camera");
      ...
      {
        preview_command_factory = make_gstreamer_preview_command_factory(GstreamerPreviewConfig{
            "gst-launch-1.0",
            preview_source_kind,
            preview_device,
      ...
      _grpc_service =
          std::make_unique<RobotGatewayService>(*_state_store, *_preview_manager, *_teleop_manager);
      _grpc_server = std::make_unique<GrpcServer>(*_grpc_service, grpc_bind_address,
                                                  static_cast<int>(grpc_port));
      _grpc_server->start();

      _vision_perf_subscription = create_subscription<omniseer_msgs::msg::VisionPerfSummary>(
          "/vision/perf", 10, [this](const omniseer_msgs::msg::VisionPerfSummary& msg)
          { _state_store->update_vision_perf(msg); });
      _odometry_subscription =
          create_subscription<nav_msgs::msg::Odometry>(robot_health_odom_topic, 10,
                                                       [this](const nav_msgs::msg::Odometry& msg)
                                                       { _state_store->update_odometry(msg); });
      _detections_subscription = create_subscription<yolo_msgs::msg::DetectionArray>(
          detections_topic, 10, [this](const yolo_msgs::msg::DetectionArray& msg)
      ...
      _teleop_poll_timer =
          create_wall_timer(std::chrono::milliseconds(50), [this]() { _teleop_manager->poll(); });
      _platform_status_timer = create_wall_timer(
          ...
          [this]()
          {
            ...
            _state_store->update_onboard_battery(
...
int main(int argc, char* argv[])
{
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("robot_diag_control_cpp"),
