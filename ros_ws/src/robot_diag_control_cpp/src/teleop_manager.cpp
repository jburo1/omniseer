...
namespace robot_diag_control_cpp
{
  namespace
  {
    uint64_t age_ms_or_zero(bool has_command, TeleopManager::SteadyTimePoint now,
  ...
  TeleopManager::TeleopManager(GatewayStateStore& store, TeleopCommandPublisher publisher,
                               TeleopManagerConfig config, TimeSource time_source)
      : _store(store), _publisher(std::move(publisher)), _config(config),
