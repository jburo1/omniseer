...
namespace robot_diag_control_cpp
{
  ...
  public:
    ...
    explicit GatewayStateStore(
        std::string gateway_name = "robot_diag_control_cpp", std::string gateway_version = "0.1.0",
  ...
  private:
    ...
    BatteryStatusSnapshot    battery_snapshot_locked(const StoredBatteryStatus& stored,
                                                     bool                       has_battery) const;
    std::vector<OperatorEventSnapshot> operator_events_snapshot_locked() const;
    void update_operator_events_locked(const VisionStatusSnapshot&   vision,
                                       const RobotHealthSnapshot&    health,
                                       const PreviewStatusSnapshot&  preview,
                                       const TeleopStatusSnapshot&   teleop,
