...
namespace robot_diag_control_cpp
{
  namespace
  {
    ...
    TEST(PreviewProcessManagerTest, PollMarksUnexpectedProcessExit)
    {
      ...
      PreviewProcessManager manager(store,
                                    make_fixed_preview_command_factory(
                                        PreviewProcessCommand{"/bin/sh", {"-c", "exit 7"}}),
    ...
    TEST(PreviewProcessManagerTest, SupportsBuiltInGstreamerTestPatternFactory)
    {
      GatewayStateStore     store;
      PreviewProcessManager manager(store,
                                    make_gstreamer_preview_command_factory(GstreamerPreviewConfig{
                                        "gst-launch-1.0",
                                        "videotest",
                                        "/dev/video11",
                                        "127.0.0.1",
                                    ...
                                    std::chrono::milliseconds(250), std::chrono::milliseconds(150));
